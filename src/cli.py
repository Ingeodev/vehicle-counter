...
Ejemplos:
  ...
  python -m mglon_vehicle_counter.cli process video.mp4 --device cuda
    ...
    process_parser.add_argument(
        "--output", "-o", default="./output", help="Carpeta de salida"
    )
    process_parser.add_argument(
        "--base-time", "-t", default="00:00", help="Hora base del video (HH:MM)"
    )
    process_parser.add_argument(
        "--device",
        "-d",
        ...
        help="Dispositivo para YOLO (default: auto-detect)",
    )
    process_parser.add_argument(
        "--model", default="yolov8s.pt", help="Modelo YOLO a usar"
    )
    process_parser.add_argument(
        "--reduce", "-r", type=int, default=1, help="Factor de reducción de resolución"
    )
    process_parser.add_argument(
        "--max-minutes", type=float, help="Límite de minutos a procesar"
    )
    process_parser.add_argument(
        "--strategy",
        choices=["box", "seg"],
    ...
    process_parser.add_argument(
        "--date",
        default="2025-01-01",  # Default provisional
    ...
    process_parser.add_argument(
        "--no-video", action="store_true", help="No guardar video de salida"
    )
    process_parser.add_argument(
        "--deblurring",
        action="store_true",
    ...
    process_parser.add_argument(
        "--quiet", "-q", action="store_true", help="Modo silencioso"
    )
    ...
    scan_parser.add_argument(
        "--output", "-o", default="./output", help="Carpeta base de salida"
    )
    scan_parser.add_argument(
        "--recursive", "-R", action="store_true", help="Escanear subdirectorios"
    )
    scan_parser.add_argument(
        "--device", "-d", default=None, choices=["cpu", "cuda", "mps"]
    )
    ...
    scan_parser.add_argument(
        "--zones", "-z", help="Ruta global al archivo JSON de zonas (override)"
    )
    ...
    scan_parser.add_argument(
        "--night-enhance", action="store_true", help="Mejorar visibilidad nocturna"
    )
    scan_parser.add_argument(
        "--date",
        default="2025-01-01",
    ...
    scan_parser.add_argument(
        "--model-config", type=str, help="Ruta a archivo YAML con parámetros del modelo"
    )
    scan_parser.add_argument(
        "--deblurring", action="store_true", help="Aplicar deblurring"
    )
    scan_parser.add_argument(
        "--yes",
        "-y",
    ...
    fix_parser.add_argument(
        "--date", required=True, help="Nueva fecha (YYYY-MM-DD o DD-MM-YYYY)"
    )
    ...
    fix_parser.add_argument(
        "--convert-h264", action="store_true", help="(Deprecado) Usar --codec h264"
    )
    fix_parser.add_argument(
        "--codec",
        choices=["copy", "h264", "h265"],
        default="copy",
        help="Codec de salida",
    )
    ...
    fix_parser.add_argument(
        "--debug", action="store_true", help="Activar modo debug (bounding box rojo)"
    )
    fix_parser.add_argument(
        "--quiet", "-q", action="store_true", help="Modo silencioso"
    )
    ...
    extract_parser = subparsers.add_parser(
        "extract-time", help="Extraer fecha/hora del OSD de videos"
    )
    ...
    extract_parser.add_argument(
        "--preprocess",
        "-p",
        choices=["clahe", "binary", "color"],
        default="clahe",
        help="Modo de preprocesamiento (default: clahe)",
    )
    extract_parser.add_argument(
        "--roi-x", type=float, default=0.5, help="Porcentaje ancho ROI (0.0-1.0)."
    )
    extract_parser.add_argument(
        "--roi-y", type=float, default=0.143, help="Porcentaje alto ROI (0.0-1.0)."
    )
    extract_parser.add_argument(
        "--corner",
        choices=["top_left", "top_right", "bottom_left", "bottom_right"],
        default="top_left",
        help="Esquina de anclaje del ROI.",
    )
    extract_parser.add_argument(
        "--output", "-o", help="Archivo CSV de salida (opcional)"
    )
    extract_parser.add_argument(
        "--export-roi", help="Directorio para guardar ROIs procesados (debug)"
    )
...
def cmd_scan(args):
    ...
    if not args.quiet:
        ...
        print(
            f"⏱️ Duración total estimada: {scan_result.total_duration_minutes / 60:.1f} horas"
        )
...
def cmd_extract_time(args):
    ...
    if not args.quiet:
        print(
            f"🔍 Extrayendo información de tiempo con OCR ({args.model}, {args.preprocess})..."
        )
        if len(args.video) > 1:
    ...
    if not args.quiet:
        print("\n" + "=" * 80)
        print(
            f"{'Video':<40} {'Fecha':<12} {'Inicio':<10} {'Fin':<10} {'Duración':<10}"
        )
        print("=" * 80)
        ...
        for info in results:
            ...
            print(
                f"{video_name:<40} {info.date:<12} {info.start_time:<10} {info.end_time:<10} {duration_str:<10}"
            )
