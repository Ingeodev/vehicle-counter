...
class VideoPipeline:
    ...
    def process_video(
        ...
        try:
            ...
            if out_cfg.save_video:
                ...
                video_writer = AnnotatedVideoWriter.from_path(output_video_path, writer_config)
            ...
            for frame_data in video.frames(max_frames=max_frames, resize=resize):
                ...
                if video_writer:
                    ...
                    # Dibujar detecciones
                    for det in detections:
                        zones_visited = zone_manager.get_zones_for_vehicle(det.track_id) if zone_manager else None
                        trajectory = counter.tracker.get_trajectory_array(det.track_id, out_cfg.max_trajectory_points)
                        self.visualizer.draw_detection(frame, det, zones_visited, trajectory)
                    
                    # Dibujar estadísticas
                    from .utils import format_timestamp
                    stats = {
                        "Detecciones": len(zone_manager.get_detection_log()) if zone_manager else 0,
            ...
            if out_cfg.save_csv and zone_manager:
                base_csv = output_video_path.replace(".mp4", "") if output_video_path else f"{output_folder}/result"
                csv_path = f"{base_csv}_detections.csv"
                self.csv_exporter.export_detections(zone_manager.get_detection_log(), csv_path)
            ...
            if out_cfg.verbose:
                self._print_end_info(processing_time, detection_log, zone_counts, output_video_path, csv_path)
    ...
    def _print_start_info(self, video_info, new_w, new_h, max_frames, zone_manager):
        ...
        print(f"📊 Resolución: {video_info.width}x{video_info.height} → {new_w}x{new_h}")
    ...
    def _print_end_info(self, processing_time, detection_log, zone_counts, video_path, csv_path):
