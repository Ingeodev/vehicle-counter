...
class YOLOSegmenter:
    ...
    def detect_and_track(self, frame: np.ndarray, persist: bool = True) -> list[Detection]:
        ...
        for i, (box, track_id, cls_id, conf) in enumerate(zip(boxes, track_ids, class_ids, confidences)):
    ...
    def detect_only(self, frame: np.ndarray) -> list[Detection]:
        ...
        results = self.model(frame, device=self.device, classes=self.allowed_class_ids, verbose=False)
    ...
    def reset_tracking(self) -> None:
        model_path = self.model.ckpt_path or self.config.model_path
        self.model = YOLO(model_path)
